...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $fields = ['first_name','middle_name','last_name','suffix','birthdate','gender',
               'civil_status','address','contact','email','educational_status',
               'school_name','employment_status','occupation','sk_voter',
    ...
    if (!empty($data['birthdate'])) {
        ...
        if ($age >= 15 && $age <= 24) {
            ...
            $errors[] = "Age must be between 15 and 30. Computed age: $age.";
        }
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $db->prepare("
            UPDATE kk_youth SET
                ...
                youth_classification=?, educational_status=?, school_name=?,
                employment_status=?, occupation=?,
                sk_voter=?, skills=?, interests=?,
...
<div class="page-header">
    <h4><i class="bi bi-pencil-square me-2"></i>Edit Youth Profile</h4>
...
<form method="POST" action="edit.php?id=<?= $id ?>" novalidate>
        ...
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" id="classification_display" class="form-control bg-light fw-semibold" readonly
                           value="<?= htmlspecialchars($data['youth_classification'] ?? '') ?>">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <select name="civil_status" class="form-select">
                        ...
                        <option value="<?= $cs ?>" <?= ($data['civil_status'] ?? 'Single') === $cs ? 'selected' : '' ?>><?= $cs ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-5">
    ...
    <div class="card content-card mb-4">
        <div class="card-header"><i class="bi bi-check2-square me-2 text-success"></i>SK Voter & Participation</div>
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-3">
                    <label class="form-label">SK Registered Voter</label>
                    <select name="sk_voter" class="form-select">
                        <option value="No"  <?= ($data['sk_voter'] ?? 'No') === 'No'  ? 'selected' : '' ?>>No</option>
                        <option value="Yes" <?= ($data['sk_voter'] ?? 'No') === 'Yes' ? 'selected' : '' ?>>Yes</option>
                    </select>
                </div>
                <div class="col-md-4">
...
updateAgeClassification(bdInput.value);
</script>
